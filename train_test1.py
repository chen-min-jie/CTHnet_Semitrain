...
def train_teacher(supervised_img=supervised_img,
                  ...
                  start_epoch=1):  # 新增 start_epoch 参数，默认从0开始
    ...
    if start_epoch > 0:
        ...
        if os.path.exists(checkpoint_path):
            ...
            start_epoch = checkpoint['epoch'] + 1  # 从下一轮继续
            print(f"从第{start_epoch}轮开始恢复训练")

    # ============== 开始训练循环 ==============
    for epoch in range(start_epoch, epochs_supervised):
        model.train()
        epoch_loss = 0
...
if __name__ == '__main__':
    output_image_dir = 'G:/20250818/origin_data/image_data'
    ...
    checkpoint_CT = "G:/20250818/Teacher_train/checkpoint_CT"

    # 从第53轮继续训练
    start_epoch = 60
    train_teacher(supervised_img=output_image_dir, supervised_lbl=output_mask_dir, region_map_dir=output_region_map_dir,
                  num_classes=num_classes, checkpoint_dir=checkpoint_dir, checkpoint_CT=checkpoint_CT,
                  lambda_soft=lambda_soft, use_boundary=False, start_epoch=start_epoch)
